...
15.dont  use this format  ```sql ```  only give me the sql query
...
def main():
    ...
    with text_tab:
        user_question = st.text_input("🔍 Type your question:", value=selected_question, key="user_question", placeholder="e.g., What are the top 5 highest-grossing movies?")
    ...
    if os.path.exists('query_logs.xlsx'):
        ...
        with open('query_logs.xlsx', 'rb') as f:
            st.download_button("Download Query History", data=f.read(), file_name="query_history.xlsx", mime="application/vnd.ms-excel", use_container_width=True)

    st.markdown("---")
    st.markdown("<div style='text-align: center; color: #666;'>🎬 Powered by Gemini and SQLite | ❤️ Sudipta</div>", unsafe_allow_html=True)
